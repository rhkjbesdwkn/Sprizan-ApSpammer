...
notifTitle.Text = "✅ Spam Completed"
...
local COMMANDS = {
	{name = "rocket", emoji = "🚀"},
	{name = "balloon", emoji = "🎈"},
	{name = "nightvision", emoji = "🌙"},
	{name = "control", emoji = "🎮"},
	{name = "jumpscare", emoji = "👻"},
	{name = "ragdoll", emoji = "🤸"},
	{name = "inverse", emoji = "🔄"},
	{name = "jail", emoji = "⛓️"},
	{name = "tiny", emoji = "🔬"},
...
local function executeCommandOnPlayer(targetPlayer, commandButtons, statusLabel)
	...
	if #commandsToExecute == 0 then
		statusLabel.Text = "⚠️ No commands enabled"
...
local function spamAllCommands(targetPlayer, statusLabel)
	task.spawn(function()
		...
		if #commandButtons == 0 then
			statusLabel.Text = "❌ No commands found"
		...
		showNotification("✅ Spam Completed", "All enabled commands executed", 3)
	end)
end
...
print("⚡ Sprizan Admin Spammer UI Loaded!")
print("🟢 Green = Enabled | 🔴 Red = Disabled")
print("📊 Loaded " .. #Players:GetPlayers() - 1 .. " players")
